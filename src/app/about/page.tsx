...
export default function AboutPage() {
  return (
    <>
      ...
      <section className="page-shell py-20 sm:py-28"><div className="max-w-3xl"><p className="eyebrow text-ink/50">The tiny things matter</p><h2 className="mt-4 font-display text-5xl leading-tight sm:text-7xl">A family signature, stitched into every tee.</h2><p className="mt-6 text-sm leading-7 text-ink/65">The bright details are deliberately small. They sit quietly on the front of each chest, with RST stitched tone-on-tone into the opposite sleeve. They are not logos for strangers — they are little signatures for the person wearing the shirt: a heart, a mustache, a bottle, and ABC blocks.</p></div><div className="mt-12 grid grid-cols-2 gap-3 md:grid-cols-4">{[["heart","#ff3fa4","Mom"],["mustache","#ff6a22","Dad"],["bottle","#20e3e3","Baby"],["blocks","#b8ff2c","Little"]].map(([type,color,label])=><div key={label} className="rounded-sm border border-ink/10 p-6"><SignatureIcon type={type as "heart"|"mustache"|"bottle"|"blocks"} color={color}/><p className="mt-8 font-display text-3xl">{label}</p></div>)}</div></section>
