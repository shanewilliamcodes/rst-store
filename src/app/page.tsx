...
import { TeeMockup } from "@/components/tee-mockup";
...
export default function HomePage() {
  return (
    <>
      ...
      <section className="bg-ink text-cream"><div className="page-shell grid items-center gap-12 py-20 lg:grid-cols-2 lg:py-28"><div className="grid grid-cols-2 gap-3">{products.map((product)=><div key={product.id} className="flex aspect-square items-center justify-center rounded-sm bg-cream/10 p-4"><TeeMockup color={product.colors[0].hex} accent={product.accent} detailType={product.detailType} snaps={product.snaps} className="h-full w-full"/></div>)}</div><div className="lg:pl-10"><p className="eyebrow text-cream/55">Look a little closer</p><h2 className="mt-4 font-display text-5xl leading-tight sm:text-7xl">Our signature stays subtle.</h2><p className="mt-5 max-w-lg text-sm leading-7 text-cream/65">Most branding shouts. Ours doesn&apos;t. Every Really Soft Tee carries a small embroidered detail on the chest in a bright little flash of neon, with RST stitched tone-on-tone into the opposite sleeve.</p><div className="mt-9 grid grid-cols-2 gap-3">{details.map((detail)=><div key={detail.label} className="border border-cream/15 p-4"><SignatureIcon type={detail.type} color={detail.color} className="h-7 w-7"/><p className="mt-4 text-xs font-bold uppercase tracking-[.14em]">{detail.label}</p><p className="mt-2 text-xs leading-5 text-cream/55">{detail.copy}</p></div>)}</div></div></div></section>
