...
export function CheckoutPanel() {
  ...
  return (
    <div className="page-shell grid gap-12 py-10 lg:grid-cols-[1fr_.75fr] lg:py-16">
      ...
      <aside className="rounded-sm bg-oatmeal/22 p-5 sm:p-7"><h2 className="text-xs font-bold uppercase tracking-[0.16em]">Order summary</h2><div className="mt-5 divide-y divide-ink/10">{items.map((item) => <div key={`${item.productId}-${item.size}-${item.color}`} className="flex gap-4 py-4"><div className="relative h-20 w-16 shrink-0 overflow-hidden bg-cream"><Image src={item.image} alt="" fill sizes="64px" className="object-cover" /></div><div className="flex-1"><div className="flex justify-between gap-3 text-sm"><span className="font-semibold">{item.name} × {item.quantity}</span><span>{formatPrice(item.price * item.quantity)}</span></div><p className="mt-1 text-xs text-ink/55">{item.color} / {item.size}</p></div></div>)}</div><div className="mt-5 space-y-3 border-t border-ink/15 pt-5 text-sm"><div className="flex justify-between"><span>Subtotal</span><span>{formatPrice(subtotal)}</span></div><div className="flex justify-between"><span>Shipping</span><span>{subtotal >= 75 ? "Free" : "Calculated"}</span></div><div className="flex justify-between border-t border-ink/15 pt-4 text-base font-bold"><span>Total</span><span>{formatPrice(subtotal)} USD</span></div></div><button className="button-primary mt-6 w-full" disabled={loading} onClick={checkout}>{loading ? <><LoaderCircle className="animate-spin" size={17} /> Opening secure checkout</> : "Continue to payment"}</button>{error && <p className="mt-4 text-sm text-red-700" role="alert">{error}</p>}</aside>
    </div>
  );
}
